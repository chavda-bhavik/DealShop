    ...
    public void beforePhase(PhaseEvent event) {
        ...
        try{
            ...
            System.out.println("In Phase Listener");
            {
                ...
                {
                    if((request.getRequestURI().contains("/customerForm") && !request.getSession().getAttribute("logged-group").equals("Admin")))
                    {
                        response.sendError(HttpServletResponse.SC_UNAUTHORIZED, "You are Not Authorised to view this Page . Go Back To Login");        
                    }
                    else if((request.getRequestURI().contains("/users/") && !request.getSession().getAttribute("logged-group").equals("Superwisor")))
                    {
                        response.sendError(HttpServletResponse.SC_UNAUTHORIZED, "You are Not Authorised to view this Page . Go Back To Login");
                    } 
                }
